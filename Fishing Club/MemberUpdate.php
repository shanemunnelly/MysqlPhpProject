...
if (isset ($_POST ['submit'])){
	...
	$exists= mysqli_query ($conn,"SELECT * FROM MEMBER WHERE IngMemNumber = '$IngMemNumber' ") or die ("Query could not be completed");
	
	// Update the location field in the DEPT table
	if (mysqli_num_rows($exists) !=0){
		mysqli_query ($conn,"UPDATE MEMBER SET strMemTitle = '$strMemTitle' WHERE IngMemNumber = '$IngMemNumber'") or die ("Update could not be applied");
...
<tr><td>Member Title:</td> <td> <input type ="text" id="strMemTitle" name="strMemTitle"> </td></tr>
