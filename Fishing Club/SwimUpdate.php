...
if (isset ($_POST ['submit'])){
	...
	$memSwimNotes = $_POST ['memSwimNotes'];
	
	
	//Check if both fields have been entered
	if ($intSwimNumber  && $memSwimNotes){
	...
	$exists= mysqli_query ($conn,"SELECT * FROM Swims WHERE intSwimNumber = '$intSwimNumber' ") or die ("Query could not be completed");
	
	// Update the location field in the DEPT table
	if (mysqli_num_rows($exists) !=0){
		mysqli_query ($conn,"UPDATE Swims SET memSwimNotes = '$memSwimNotes' WHERE intSwimNumber = '$intSwimNumber'") or die ("Update could not be applied");
...
<tr><td>Swim Notes:</td> <td> <input type ="text" id="memSwimNotes" name="memSwimNotes"> </td></tr>
